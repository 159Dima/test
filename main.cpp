...
class IteratorRange {
public:
    IteratorRange(Iter begin, Iter end): iter_begin_(begin), iter_end_(end){}
    ...
    size_t size() const {
        return distance(iter_begin_, iter_end_);
    }
private:
...
public:
    Paginator(Iterator begin, Iterator end, size_t size) {
        ...
        while (i < end) {
            ...
            list.push_back(IteratorRange(i, new_i));
            i = new_i;
        }
    }
    auto begin() const {
...
private:
    vector<IteratorRange<Iterator>>list;
};


ostream& operator<<(ostream& out, const Document& doc) {
    out << "{"s << "document_id = "s << doc.id << ", "s
...
template <typename Iterator>
ostream& operator<<(ostream& out, const IteratorRange<Iterator>& list) {
    auto pages = list.begin();
    for (; pages != list.end(); ++pages) {
        out << *pages;
    }
    return out;
}
